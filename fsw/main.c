 ...
 * 
 * Sorumluluğu:
 * 1. Hardware initialization (STM32H743IIK6 HAL, UART, I2C, SPI, RTC, Watchdog)
 ...
 */
static int32_t system_init_hardware(void) {
    printf("[BOOT] Initializing STM32H743IIK6 Hardware...\n");

    /* Actual STM32H743IIK6 full HAL initialization.
     * Requires STM32CubeMX generated source files (stm32h7xx_it.c/.h, stm32h7xx_hal_conf.h, system_stm32h7xx.c, startup_stm32h743xx.s)
     * and correct linker script for STM32H743IIK6 (flash: 2MB, sram1+2: 1024KB etc).
     */
    HAL_Init();
    SystemClock_Config();
    // Initialization functions generated by CubeMX or manual HAL setup:
    // MX_GPIO_Init();
    // MX_USART2_UART_Init();
    // MX_USART3_UART_Init();
    // MX_USART4_UART_Init();
    ...
    /* Mock fallback logging for current integration stage */

    printf("[BOOT] ✓ STM32H743IIK6 mock hardware initialization complete\n");
...
/**
 * @brief System clock configuration for STM32H743IIK6 (stub)
 *
 * Replace with CubeMX-generated SystemClock_Config() for the target hardware.
 */
void SystemClock_Config(void) {
    // Placeholder: real code must configure HSE/HSI, PLL and bus clocks.
    // For full integration, import CubeMX contents into fsw/system_stm32h7xx.c
}
...
/**
 * @brief Main function - STM32H743IIK6's entry point
 * 
 * Execution flow:
 * 1. Hardware init
